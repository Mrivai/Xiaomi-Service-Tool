...
namespace Qualcomm_Imei_Changer
{
    public partial class ImeiChanger : MetroForm
    {
        ...
        private void ActivatedDiagMode()
        {
            ...
            else if (devices.State == DeviceState.ONLINE || devices.State == DeviceState.RECOVERY)
            {
                if (!devices.IsDiag)
                {
                    ...
                    kenzo.SpeakAsync("reading device international  mobility equiptment indentification number");
                }
                Read();
            }
        }

        private void ImeiChanger_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (devices.IsDiag)
            {
